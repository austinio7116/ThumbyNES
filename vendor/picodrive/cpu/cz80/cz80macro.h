...
#if PICODRIVE_HACKS
/* ThumbyNES: on Cortex-M33 (Thumb-only) function pointers carry the
 * Thumb bit — packing via (addr >> 1) | MAP_FLAG discards it, so we
 * OR it back at dispatch. Matches cpu68k_map's MAP_FP and the same
 * fix in cz80.c's picodrive_read. */
#ifdef __thumb__
# define CZ80_WR_MAP_FP(v)  (((v) << 1) | 1)
#else
# define CZ80_WR_MAP_FP(v)  ((v) << 1)
#endif
#define WRITE_MEM8(A, D) { \
	...
	if (map_flag_set(v)) \
		((z80_write_f *)CZ80_WR_MAP_FP(v))(a, d); \
	else \
