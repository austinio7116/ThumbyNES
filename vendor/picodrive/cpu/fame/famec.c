...
/* ThumbyNES: JumpTable is 64K × 4 bytes = 256 KB of BSS. Three modes:
 *
 *  FAME_JUMPTABLE_IN_FLASH — device build. The table is precomputed
 *      by tools/gen_md_fame_jumptable.c at build time and linked as
 *      const data. GCC resolves each OP_0xYYYY to its link-time
 *      address. Zero SRAM cost. init_jump_table() becomes a no-op.
 *
 *  default — heap-allocate on first init_jump_table(), free via
 *      fm68k_shutdown(). Matches the host bench where RAM is free. */
#ifdef FAME_JUMPTABLE_IN_FLASH
/* The table itself is defined below, AFTER `#include
 * "famec_opcodes.h"` has defined all OP_0xYYYY functions — that's
 * the only way to reference them in a const initialiser without
 * changing their static-visibility. Here we just expose JumpTable
 * to fm68k_emulate's read loop. */
extern const opcode_func JumpTable_ro[0x10000];
#define JumpTable ((opcode_func *)JumpTable_ro)
#else
...
/* ThumbyNES: after every OP_0xYYYY has been defined above, we can
 * take their addresses as link-time constants in a `const` array.
 * This is what moves the 256 KB JumpTable into flash. */
#ifdef FAME_JUMPTABLE_IN_FLASH
const opcode_func JumpTable_ro[0x10000] = {
#include "famec_jumptable_data.inc"
};
#endif
#endif
...
#ifdef FAME_JUMPTABLE_IN_FLASH
	/* Table is const flash data; nothing to alloc or fill. */
#else
	if (!JumpTable) {
...
#include "famec_jumptable_init.inc"
#endif
#if 0   /* ThumbyNES: init-body replaced by the .inc above; keep the
         * original source below, fenced out, so `git diff` still
         * shows the two variants side by side. */
...
#endif  /* end fenced-out original init body */
...
{
#ifndef FAME_JUMPTABLE_IN_FLASH
	free(JumpTable);
	JumpTable = NULL;
#endif
	initialised = 0;
