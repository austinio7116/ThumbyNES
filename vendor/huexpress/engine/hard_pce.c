...
hard_init(void)
{
    /* ThumbyNES patch: upstream hard-codes 8*4 = 32 bytes assuming a
     * 32-bit target (ESP32 LX6). On 64-bit host pointers are 8 bytes
     * and the allocation overflows into the next heap block, clobbering
     * PageW[0..3] when PageR[4..7] are written. Use sizeof(uchar*)
     * so both host (64-bit) and device (32-bit Cortex-M33) get right. */
    PageR = (uchar **)my_special_alloc(true, 1, 8 * sizeof(uchar *));
