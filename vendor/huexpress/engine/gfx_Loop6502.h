...
jump_CheckSprites:
            ...
            if (!UCount) {
                ...
                UCount = UPeriod;
                /* ThumbyNES patch: signal exe_go's outer while-loop to
                 * return so our pcec_run_frame can hand back control
                 * once per display refresh. Upstream relied on a
                 * separate task thread for frame pacing. */
                {
