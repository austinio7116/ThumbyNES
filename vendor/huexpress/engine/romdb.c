...
/* ThumbyNES patch: when the wrapper has seeded an in-memory ROM
 * (g_pce_mem_rom_active != 0), return the precomputed CRC instead of
 * trying to open `name` — the synthetic rom_file_name isn't a real
 * file and there's no filesystem to open it on. The wrapper computes
 * g_pce_mem_rom_crc over the exact bytes InitPCE will consume. */
extern int    g_pce_mem_rom_active;
extern uint32 g_pce_mem_rom_crc;

uint32
CRC_file(char *name)
{
	if (g_pce_mem_rom_active) return g_pce_mem_rom_crc;
