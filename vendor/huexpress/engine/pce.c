...
InitPCE(char *name)
{
	...
	if (ROM[0x1FFF] < 0xE0) {
		/* ThumbyNES patch: originally decrypted the ROM in place here,
		 * but our ROM buffer lives in read-only XIP flash on device.
		 * Decryption is the wrapper's responsibility — see
		 * pcec_rom_is_us_encoded / pcec_rom_decrypt_us. If we reach
		 * this branch it means the wrapper missed a US cart; log it
		 * and continue (game will render garbage, diagnostic rather
		 * than a silent failure). */
		Log("PCE: ROM[0x1FFF]=%02X looks US-encoded but wrapper didn't decrypt — expect garbage\n",
		    ROM[0x1FFF]);
		local_us_encoded_card = 1;
	}
	/* Upstream's in-place nibble-reverse decrypt loop removed — see
	 * VENDORING.md. `local_us_encoded_card` is retained because other
	 * InitPCE paths may still read it (currently no-op but future-
	 * proof). */
/*
	if (CD_emulation)
		{
