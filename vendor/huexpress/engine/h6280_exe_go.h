...
/*    flnz_list = (uchar *)my_special_alloc(false, 1,256);
  ...
  */
  /* ThumbyNES patch: upstream exe_go() was designed to never return;
   * gfx_init() runs once at entry. My frame-done patch re-enters
   * exe_go() every frame, which would reset VDC CR to 0 and kill any
   * VBL-enable the game just wrote (see bug trace in PCE_PLAN.md).
   * The wrapper calls gfx_init() once at pcec_load_rom() instead. */
  /* gfx_init(); */
    uchar I;
    GFX_Loop6502_Init
...
//#undef reg_a
 //   uchar reg_a = 0;
    ...
    /* ThumbyNES patch: break out of the forever-loop once the
     * periodic handler signals end-of-frame. Upstream relied on a
     * dedicated CPU task under FreeRTOS; we drive the core per-frame
     * from the caller. */
    extern volatile int g_pce_frame_done;
    g_pce_frame_done = 0;
    while (!g_pce_frame_done) {
