 ...
 * 1 KB byte-indexed half-tables suffice. tile_get composes them at
 * lookup time via the BP_LUT() macro below. */
static uint32 bp_lut_lo[0x100];
static uint32 bp_lut_hi[0x100];
#define BP_LUT(idx)  (bp_lut_lo[(idx) & 0xff] | bp_lut_hi[(idx) >> 8])
...
void render_init(void)
{
  ...
  /* THUMBYNES PATCH: populate the two 1 KB half-tables. */
  for (i = 0; i < 0x100; i++) {
    uint32 lo = 0, hi = 0;
    for (int x = 0; x < 8; x++) {
      if (i & (0x80 >> x)) lo |= (uint32)(4u << (x << 2));
      if (i & (0x80 >> x)) hi |= (uint32)(8u << (x << 2));
    }
    bp_lut_lo[i] = lo;
    bp_lut_hi[i] = hi;
  }
...
/* THUMBYNES PATCH: padded from [8] to [16].
 * render_bg_sms / render_bg_tms read this via cache_ptr as two
 * adjacent uint32s, and the compiler at -O3 fuses those two reads
 * into a single 8-byte load that starts at &cache_ptr[1] = &data[4]
 * and runs to &data[12] — 4 bytes off the end of the original [8].
 * Upstream gets away with it because the bytes past &data[8] are
 * usually zero on the typical BSS layout, but as soon as anything
 * non-zero ends up there (e.g. our static _lut[0x10000]) the
 * garbage pixel indices propagate into the pixel pipeline and the
 * VDP eventually walks a bad pointer. Padding to 16 keeps the
 * fused load entirely in-bounds. */
...
void palette_sync(int index)
{
  ...
  /* THUMBYNES PATCH: dropped retro-go's byteswap into BE — both SDL
   * (PIXELFORMAT_RGB565) and the Thumby LCD GC9107 driver consume
   * native-endian little-endian RGB565. */
