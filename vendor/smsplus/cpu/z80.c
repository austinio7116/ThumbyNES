...
/* THUMBYNES PATCH: was `static UINT8 *SZHVC_add = 0;` plus a runtime
 * malloc + init in z80_init(). The tables are 256 KB total and the
 * RP2350 device build can't spare that much heap; precomputed into
 * flash via tools/gen_sms_z80_szhvc.c. */
extern const UINT8 sms_szhvc_add_data[2*256*256];
extern const UINT8 sms_szhvc_sub_data[2*256*256];
#define SZHVC_add sms_szhvc_add_data
#define SZHVC_sub sms_szhvc_sub_data
...
void z80_init(int index, int clock, const void *config, int (*irq_callback)(int))
{
  ...
  /* THUMBYNES PATCH: SZHVC_add / SZHVC_sub are now flash-resident
   * const arrays generated by tools/gen_sms_z80_szhvc.c. The original
   * 256 KB malloc + init loop has been removed entirely. */
...
void z80_exit(void)
{
  /* THUMBYNES PATCH: SZHVC tables now live in flash, nothing to free. */
}
