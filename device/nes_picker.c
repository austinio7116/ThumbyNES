...
#  ifdef THUMBYNES_WITH_GBA
        else if (strcasecmp(info.fname + L - 4, ".gba") == 0) sys = ROM_SYS_GBA;
#  endif
        else if (L >= 3 && strcasecmp(info.fname + L - 3, ".gb")  == 0) sys = ROM_SYS_GB;
        else if (L >= 3 && strcasecmp(info.fname + L - 3, ".gg")  == 0) sys = ROM_SYS_GG;
        else continue;
#else
        /* GBA-only build: the picker only surfaces .gba files. */
        if (strcasecmp(info.fname + L - 4, ".gba") == 0) sys = ROM_SYS_GBA;
        else continue;
#endif
...
/* fb helpers + colour palette used across the picker, hero, and list
 * draw paths below. */
...
static void format_meta(char *out, size_t outsz, const nes_rom_entry *e) {
    ...
    if      (e->system == ROM_SYS_SMS) snprintf(out, outsz, "SMS  %luK  %s",
                                                 ...
                                                 (unsigned long)(e->size / 1024));
    else if (e->system == ROM_SYS_GBA) snprintf(out, outsz, "GBA  %luK",
                                                 (unsigned long)(e->size / 1024));
...
static int build_view(const nes_rom_entry *entries, int n_entries,
    ...
    for (int i = 0; i < n_entries; i++) {
        ...
        case TAB_GBA: if (entries[i].system != ROM_SYS_GBA) continue; break;
        }
        view[n++] = i;
    }
    sort_mode_g    = sort;
...
static void tab_counts(const nes_rom_entry *e, int n, int counts[TAB_COUNT]) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        else if (e[i].system == ROM_SYS_GBA) counts[TAB_GBA]++;
    }
}
...
static void draw_tab_bar(uint16_t *fb, int active_tab, const int counts[TAB_COUNT]) {
    static const uint8_t icon_for[TAB_COUNT] = {
        ...
        /* No bespoke GBA icon yet — reuse the GB tab icon so the strip
         * still draws cleanly. Replace once tools/tab_icons/gba.png is
         * authored and convert_tab_icons.py regenerates the .inc. */
        ICON_SYS_GB,
    };
...
static void draw_hero(uint16_t *fb, const nes_rom_entry *e, int sel,
    ...
    static const char * const tab_label[TAB_COUNT] = {
        ...
        "MEGA DRIVE", "PC ENGINE", "GAME BOY ADVANCE",
    };
