...
void tud_msc_capacity_cb(uint8_t lun, uint32_t *block_count, uint16_t *block_size) {
    (void)lun;
    *block_count = nes_flash_disk_sector_count();
    *block_size  = (uint16_t)nes_flash_disk_sector_size();
}
...
int32_t tud_msc_read10_cb(uint8_t lun, uint32_t lba, uint32_t offset,
    ...
    uint32_t sector_size = nes_flash_disk_sector_size();
    if (bufsize % sector_size != 0) return -1;
    uint32_t count = bufsize / sector_size;
    if (nes_flash_disk_read((uint8_t *)buffer, lba, count) != 0) return -1;
...
int32_t tud_msc_write10_cb(uint8_t lun, uint32_t lba, uint32_t offset,
    ...
    if (bufsize % sector_size != 0) return -1;
    uint32_t count = bufsize / sector_size;
    if (nes_flash_disk_write(buffer, lba, count) != 0) return -1;
    return (int32_t)bufsize;
}
