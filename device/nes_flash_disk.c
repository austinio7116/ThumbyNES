...
uint32_t nes_flash_disk_sector_size (void) { return FLASH_DISK_SECTOR_SIZE; }
...
#ifdef THUMBYONE_SLOT_MODE
/* Save the ATRANS mapping the bootrom set up when it chained us
 * into this partition, so we can restore it after the SDK's flash
 * erase/program routines reset QMI. Must be called with IRQs
 * disabled if the surrounding flash op also disables IRQs. */
...
void __not_in_flash_func(nes_flash_disk_flush)(void) {
    for (int i = 0; i < CACHE_BLOCKS; i++) {
...
int __not_in_flash_func(nes_flash_disk_commit_one)(void) {
    int oldest = -1;
...
uint32_t nes_flash_disk_stat_writes       (void) { return stat_writes;        }
uint32_t nes_flash_disk_stat_commits      (void) { return stat_commits;       }
uint32_t nes_flash_disk_stat_commit_errors(void) { return stat_commit_errors; }
uint32_t nes_flash_disk_stat_dirty_n(void) {
    uint32_t n = 0;
